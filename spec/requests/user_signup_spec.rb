  ...
  describe "User creates a new account" do
    ...
    context "When user passes invalid credentials as part of the request" do
      it "doesn't create a user account" do
        ...
        post user_new_path(password: "12345678"), {}, { "Accept" => "application/json" }
        expect(response).to have_http_status(422)

        post user_new_path(username: "Ikem", password: "12"), {}, { "Accept" => "application/json" }
        expect(response).to have_http_status(422)

        post user_new_path(username: "Ikem", password: "12345678"), {}, { "Accept" => "application/json" }
        expect(response).to have_http_status(201)
